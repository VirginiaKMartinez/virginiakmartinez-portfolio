...
- **Case study 04 — Hawi**: Route `/case/hawi`, page `src/pages/CaseHawi.jsx`. Product skin in `src/styles/hawi.css` with `--hw-*` tokens. Components in `src/components/hawi/`: HawiHero, OhanaFlow, BrandKit, ActorCards, ProcessThree, MockupStage, DeviceFrame, HawiLogomark. Fonts: Nunito (body) + Caveat (sketch device), loaded in `index.html`. Images in `public/case/hawi/`. Translations at `case.hawi.*` in both EN and FR. Process devices (SketchDevice, BlockframeDevice, HiFiDevice) are drawn with CSS — no images. `OtherCases` accepts `exclude` prop; call `<OtherCases exclude="hawi" />` from the Hawi page. `BackFoot` accepts optional `mid` prop for per-case footer text.
...
*Last updated: 2026-05-26. Living document. Update Decisions Already Closed every time we close a new one.*
