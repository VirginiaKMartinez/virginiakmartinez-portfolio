...
1. **Selected Work square thumbs (WIP, the ONLY thing that looks unfinished)**: Virginia's uncommitted work in `SelectedWork.jsx` + `editorial.css` renders dashed placeholder boxes with the case slug (`STAFF-AND-GO`, `FICHE-SALARIE`, `HAWI`, `GAZELLA`). Needs 4 square images (exports from Virginia, or crops of existing case visuals). Close this, run `npm run build` once more (should now be warning-free), then ship.
