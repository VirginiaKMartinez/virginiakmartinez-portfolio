...
- **Shared case components**: `Chapter`, `StatsRow`, `CaseTopbar`, `OtherCases` (accepts `exclude`), `BackFoot` (accepts `mid`), `ScrollPhone` live in `components/case/`. `DeviceFrame` (`components/hawi/`) is cross-case reusable. NOTE: `ScrollPhone` was moved out of `components/gazella/` into `components/case/` (2026-07); its styles moved from `gazella.css` (`.gz-scrollphone`) to `case.css` (neutral `.scrollphone`, `--gz-*` refs replaced by their literal fallbacks `#fff`/`#2b2b2b` — zero visual change). The scroll-hint pill is one line with an SVG up/down icon. `OtherCases` `CASES` array + `case.other.*` keys list all four cases; its title is count-agnostic ("More work, / in brief.").
